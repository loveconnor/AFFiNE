...
const { ok } = await new WebClient(SLACK_BOT_TOKEN).chat.postMessage({
  channel: CHANNEL_ID,
  text: `LoveNotes Copilot Test ${COPILOT_RESULT}`,
  blocks: render(
    `# LoveNotes Copilot Test ${COPILOT_RESULT}

- [${BRANCH_NAME?.replace('refs/heads/', '') || BRANCH_SHA}](https://github.com/toeverything/LoveNotes/commit/${BRANCH_SHA})
`
  ),
});
