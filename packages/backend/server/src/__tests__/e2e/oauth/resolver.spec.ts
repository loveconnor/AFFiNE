import { oauthProvidersQuery } from '@lovenotes/graphql';
...
e2e('should return apple oauth provider in version >= 0.22.0', async t => {
  ...
  const res2 = await app.gql({
    ...
    context: {
      headers: {
        'x-lovenotes-version': '0.23.0-beta.1',
      },
    },
  });
  ...
  const res3 = await app.gql({
    ...
    context: {
      headers: {
        'x-lovenotes-version': '2025.6.29-canary.93',
      },
    },
  });
...
e2e(
  ...
  async t => {
    const res = await app.gql({
      ...
      context: {
        headers: {
          'x-lovenotes-version': 'mock-invalid-version',
        },
      },
    });
