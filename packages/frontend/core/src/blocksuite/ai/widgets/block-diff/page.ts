...
export const AFFINE_BLOCK_DIFF_WIDGET_FOR_PAGE =
  'lovenotes-block-diff-widget-for-page';

export class LoveNotesBlockDiffWidgetForPage extends WidgetComponent {
  ...
  _handleScroll(dir: 'prev' | 'next') {
    ...
    const diffWidgets = Array.from(
      this.std.host.querySelectorAll('lovenotes-block-diff-widget-for-block')
    );
