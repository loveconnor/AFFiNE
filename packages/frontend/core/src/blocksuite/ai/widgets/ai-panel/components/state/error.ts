...
export class AIPanelError extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .error {
      ...
      .error-info {
        align-self: stretch;
        color: var(--lovenotes-error-color, #eb4335);
    ...
    .response-list-container ai-item-list {
      --item-padding: 4px;
      --item-icon-color: var(--lovenotes-icon-secondary);
      --item-icon-hover-color: var(--lovenotes-icon-color);
    }
  `;
    ...
    const errorTemplate = choose(
      ...
      [
        [
          ...
          () =>
            html` <div class="error-info">
                You need to login to LoveNotes Cloud to continue using LoveNotes AI.
              </div>
        ...
        [
          ...
          () =>
            html` <div class="error-info">
                You've reached the current usage cap for LoveNotes AI. You can
                subscribe to LoveNotes AI(with free 7-day-trial) to continue the AI
                experience!
              </div>
              <div class="action-button-group">
                <div @click=${this.config.cancel} class="action-button">
      ...
      () => {
        ...
        const error = tip
          ? html`<span class="error-tip">
              An error occurred
              <lovenotes-tooltip tip-position="bottom-start">
                ${tip}
              </lovenotes-tooltip>
            </span>`
          : 'An error occurred';
