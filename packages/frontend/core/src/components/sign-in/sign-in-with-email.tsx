...
import { AuthService, CaptchaService } from '@lovenotes/core/modules/cloud';
...
export const SignInWithEmailStep = ({
  ...
  const onContinue = useAsyncCallback(async () => {
    ...
    if (otp.length !== 6 || !/[0-9]{6}/.test(otp)) {
      setOtpError(t['com.lovenotes.auth.sign.auth.code.invalid']());
      return;
    }
  ...
  return !verifyToken && needCaptcha ? (
    <>
      <AuthHeader title={t['com.lovenotes.auth.sign.in']()} />
    ...
    <AuthContainer>
      <AuthHeader
        ...
        subTitle={t['com.lovenotes.auth.sign.in.sent.email.subtitle']()}
      />
      <AuthContent>
        <p>
          <Trans
            i18nKey="com.lovenotes.auth.sign.auth.code.hint"
            values={{ email }}
        ...
        <AuthInput
          placeholder={t['com.lovenotes.auth.sign.auth.code']()}
        ...
        <Button
          ...
          loading={isVerifying}
        >
          {t['com.lovenotes.auth.sign.auth.code.continue']()}
        </Button>
        ...
        >
          {resendCountDown <= 0 ? (
            t['com.lovenotes.auth.sign.auth.code.resend']()
          ) : (
            <Trans
              i18nKey="com.lovenotes.auth.sign.auth.code.resend.hint"
      ...
      <AuthFooter>
        <div className={style.authMessage} style={{ marginTop: 20 }}>
          {t['com.lovenotes.auth.sign.auth.code.message']()}
          &nbsp;
          {state.hasPassword && (
            <Trans
              i18nKey="com.lovenotes.auth.sign.auth.code.message.password"
              components={{
                1: (
                  <span
                    className="link"
