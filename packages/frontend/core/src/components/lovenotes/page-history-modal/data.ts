...
export const useSnapshotPage = (
  ...
  const lovenotesWorkspace = useService(WorkspaceService).workspace;
  ...
  const page = useMemo(() => {
    ...
    const historyShellWorkspace = getOrCreateShellWorkspace(
      docCollection.id,
      workspacesService.getWorkspaceFlavourProvider(lovenotesWorkspace.meta)
    );
