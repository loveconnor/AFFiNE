...
import { Modal, useConfirmModal } from '@lovenotes/component/ui/modal';
...
import { LoveNotesErrorBoundary } from '../lovenotes-error-boundary';
import {
      ...
      <div className={styles.previewContent}>
        ...
        {snapshotPage ? (
          <LoveNotesErrorBoundary>
            <Scrollable.Root>
              <Scrollable.Viewport className="lovenotes-page-viewport">
                <BlockSuiteEditor
...
const PlanPrompt = () => {
      ...
      <div className={styles.planPromptTitle}>
        {
          isProWorkspace !== null
            ? !isProWorkspace
              ? t[
                  'com.lovenotes.history.confirm-restore-modal.plan-prompt.limited-title'
                ]()
              : t[
                  'com.lovenotes.history.confirm-restore-modal.plan-prompt.title'
                ]()
  ...
  const planDescription = useMemo(() => {
    if (!isProWorkspace) {
      return (
        <>
          <Trans i18nKey="com.lovenotes.history.confirm-restore-modal.free-plan-prompt.description">
          ...
          {isOwner ? (
            ...
            >
              {t[
                'com.lovenotes.history.confirm-restore-modal.pro-plan-prompt.upgrade'
              ]()}
            </span>
          ) : null}
        </>
      ...
      return (
        <Trans i18nKey="com.lovenotes.history.confirm-restore-modal.pro-plan-prompt.description">
  ...
  return (
    <div className={styles.historyList}>
      <div className={styles.historyListHeader}>
        {t['com.lovenotes.history.version-history']()}
      </div>
                    ...
                    return (
                      <Fragment key={history.timestamp}>
                        ...
                        {isLastGroup && isLastItem && onLoadMore ? (
                          ...
                          >
                            {t[
                              'com.lovenotes.history.confirm-restore-modal.load-more'
                            ]()}
                          </Button>
                        ) : null}
                      </Fragment>
  ...
  return (
    ...
    >
      ...
      <div className={styles.emptyHistoryPromptTitle}>
        {t['com.lovenotes.history.empty-prompt.title']()}
      </div>
      <div className={styles.emptyHistoryPromptDescription}>
        {t['com.lovenotes.history.empty-prompt.description']()}
      </div>
    </div>
  );
};
...
const PageHistoryManager = ({
  ...
  const onConfirmRestore = useCallback(() => {
    openConfirmModal({
      title: t['com.lovenotes.history.restore-current-version'](),
      description: t['com.lovenotes.history.confirm-restore-modal.hint'](),
      cancelText: t['Cancel'](),
      contentOptions: {
        ['data-testid' as string]: 'confirm-restore-history-modal',
        style: { padding: '20px 26px' },
      },
      confirmText: t['com.lovenotes.history.confirm-restore-modal.restore'](),
    ...
    <div className={styles.root}>
      ...
      <div className={styles.historyFooter}>
        <Button onClick={onClose}>
          {t['com.lovenotes.history.back-to-page']()}
        </Button>
        ...
        >
          {t['com.lovenotes.history.restore-current-version']()}
        </Button>
      </div>
    </div>
  );
