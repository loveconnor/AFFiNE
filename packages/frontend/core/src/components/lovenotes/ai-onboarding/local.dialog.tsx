...
export const AIOnboardingLocal = () => {
  ...
  const actions = useMemo(() => {
    ...
    if (notSignedIn) {
      result.push({
        key: 'get-started',
        label: t['com.lovenotes.ai-onboarding.local.action-get-started'](),
        onClick: () => {
      ...
      const id = notify(
        {
          title: (
            <div className={styles.title}>
              {t['com.lovenotes.ai-onboarding.local.title']()}
            </div>
          ),
          message: t['com.lovenotes.ai-onboarding.local.message'](),
          icon: <AiIcon />,
