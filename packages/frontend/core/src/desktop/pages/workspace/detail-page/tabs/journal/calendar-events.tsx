import { Loading, toast, Tooltip } from '@lovenotes/component';
...
const CalendarEventRenderer = ({ event }: { event: CalendarEvent }) => {
  ...
  const handleClick = useAsyncCallback(async () => {
    ...
    if (docs.length === 0) {
      toast(
        t['com.lovenotes.integration.calendar.no-journal']({
      ...
      {loading ? (
        ...
        <div className={styles.eventCaption}>
          <span className={styles.eventTime}>
            {allDay
              ? t['com.lovenotes.integration.calendar.all-day']()
              : formatTime(startAt, endAt)}
          </span>
          <span className={styles.eventNewDoc}>
            <PlusIcon style={{ fontSize: 18 }} />
            {t['com.lovenotes.integration.calendar.new-doc']()}
          </span>
        </div>
      )}
    </li>
