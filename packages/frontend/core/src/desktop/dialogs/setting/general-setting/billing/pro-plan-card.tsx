...
const CloudExpirationInfo = () => {
  ...
  if (subscription?.status === SubscriptionStatus.PastDue) {
    text = t['com.lovenotes.payment.billing-tip.past-due']({
      due: i18nTime(subscription.nextBillAt, {
        absolute: { accuracy: 'day' },
      }),
    });
  } else if (subscription?.nextBillAt) {
    text = t['com.lovenotes.payment.billing-setting.renew-date.description']({
      renewDate: i18nTime(subscription.nextBillAt, {
        absolute: { accuracy: 'day' },
      }),
    });
  } else if (subscription?.end) {
    text = t['com.lovenotes.payment.billing-setting.due-date.description']({
    ...
    <div className={styles.planActionContainer}>
      ...
      >
        {plan === SubscriptionPlan.Pro
          ? t['com.lovenotes.payment.billing-setting.change-plan']()
          : t['com.lovenotes.payment.billing-setting.upgrade']()}
      </Button>
      {subscriptionStatus === SubscriptionStatus.PastDue ? (
