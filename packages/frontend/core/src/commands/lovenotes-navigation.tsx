import type { useI18n } from '@lovenotes/i18n';
import { track } from '@lovenotes/track';
import type { Workspace } from '@blocksuite/lovenotes/store';
import { ArrowRightBigIcon } from '@blocksuite/icons/rc';

import type { useNavigateHelper } from '../components/hooks/use-navigate-helper';
import type { WorkspaceDialogService } from '../modules/dialogs';
import type { WorkbenchService } from '../modules/workbench';
import { registerLoveNotesCommand } from './registry';

export function registerLoveNotesNavigationCommands({
  t,
  docCollection,
  navigationHelper,
  workspaceDialogService,
  workbenchService,
}: {
  t: ReturnType<typeof useI18n>;
  navigationHelper: ReturnType<typeof useNavigateHelper>;
  docCollection: Workspace;
  workspaceDialogService: WorkspaceDialogService;
  workbenchService?: WorkbenchService;
}) {
  const unsubs: Array<() => void> = [];
  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:goto-all-pages',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: t['com.lovenotes.cmdk.lovenotes.navigation.goto-all-pages'](),
      run() {
        track.$.cmdk.navigation.navigate({
          to: 'allDocs',
        });

        navigationHelper.jumpToPage(docCollection.id, 'all');
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:goto-collection-list',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: 'Go to Collection List',
      run() {
        track.$.cmdk.navigation.navigate({
          to: 'collectionList',
        });

        navigationHelper.jumpToCollections(docCollection.id);
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:goto-tag-list',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: 'Go to Tag List',
      run() {
        track.$.cmdk.navigation.navigate({
          to: 'tagList',
        });

        navigationHelper.jumpToTags(docCollection.id);
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:goto-workspace',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: t['com.lovenotes.cmdk.lovenotes.navigation.goto-workspace'](),
      run() {
        track.$.cmdk.navigation.navigate({
          to: 'workspace',
        });

        workbenchService?.workbench.openWorkspaceSelector();
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:open-settings',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: t['com.lovenotes.cmdk.lovenotes.navigation.open-settings'](),
      keyBinding: '$mod+,',
      run() {
        track.$.cmdk.settings.openSettings();
        workspaceDialogService.open('setting', {
          activeTab: 'appearance',
        });
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:open-account',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: t['com.lovenotes.cmdk.lovenotes.navigation.open-account-settings'](),
      run() {
        track.$.cmdk.settings.openSettings({ to: 'account' });
        workspaceDialogService.open('setting', {
          activeTab: 'account',
        });
      },
    })
  );

  unsubs.push(
    registerLoveNotesCommand({
      id: 'lovenotes:goto-trash',
      category: 'lovenotes:navigation',
      icon: <ArrowRightBigIcon />,
      label: t['com.lovenotes.cmdk.lovenotes.navigation.goto-trash'](),
      run() {
        track.$.cmdk.navigation.navigate({
          to: 'trash',
        });

        navigationHelper.jumpToPage(docCollection.id, 'trash');
      },
    })
  );

  return () => {
    unsubs.forEach(unsub => unsub());
  };
}
