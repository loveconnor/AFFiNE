...
export class LoveNotesUpdateProvider extends Provider<GithubUpdateInfo> {
  static configFeed(options: UpdateProviderOptions): CustomPublishOptions {
    return {
      provider: 'custom',
      feedUrl: 'https://lovenotes.pro/api/worker/releases',
      updateProvider: LoveNotesUpdateProvider,
      ...options,
    };
  }
...
type FileParts =
  | ['lovenotes', string, VersionDistribution, VersionPlatform, VersionArch]
  | [
      'lovenotes',
      string,
      `${'canary' | 'beta'}.${number}`,
...
export function availableForMyPlatformAndInstaller(
  ...
  //  in form of:
  //   lovenotes-${build}-${buildSuffix}-${distribution}-${platform}-${arch}.${installer}
