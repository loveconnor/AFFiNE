...
let protocol = buildType === 'stable' ? 'lovenotes' : `lovenotes-${buildType}`;
if (isDev) {
  protocol = 'lovenotes-dev';
}
...
export function setupDeepLink(app: App) {
  ...
  app.on('open-url', (event, url) => {
    ...
    if (url.startsWith(`${protocol}://`)) {
      ...
      app
        .whenReady()
        .then(() => handleLoveNotesUrl(url))
        .catch(e => {
          logger.error('failed to handle lovenotes url', e);
        });
    }
  });
  ...
  app.on('second-instance', (event, commandLine) => {
    getMainWindow()
      .then(window => {
        ...
        if (url?.startsWith(`${protocol}://`)) {
          event.preventDefault();
          handleLoveNotesUrl(url).catch(e => {
            logger.error('failed to handle lovenotes url', e);
          });
        }
      })
...
async function handleLoveNotesUrl(url: string) {
  await showMainWindow();

  logger.info('open lovenotes url', url);
