...
vi.doMock('@lovenotes/electron/helper/db/ensure-db', () => ({
  ensureSQLiteDB: async () => ({
    destroy: () => {},
  }),
}));

vi.doMock('@lovenotes/electron/helper/main-rpc', () => ({
  mainRPC: {
...
afterAll(() => {
  vi.doUnmock('@lovenotes/electron/helper/main-rpc');
});

describe('workspace db management', () => {
  test('trash workspace', async () => {
    const { trashWorkspace } =
      await import('@lovenotes/electron/helper/workspace/handlers');
