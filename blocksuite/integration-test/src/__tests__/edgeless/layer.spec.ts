...
import { ungroupCommand } from '@blocksuite/lovenotes/gfx/group';
import type {
  GroupElementModel,
  NoteBlockModel,
} from '@blocksuite/lovenotes/model';
import { generateKeyBetween } from '@blocksuite/lovenotes/std/gfx';
  ...
  const assertBlocksContent = () => {
    const blocks = Array.from(
      document.querySelectorAll(
        'lovenotes-edgeless-root gfx-viewport > [data-block-id]'
      )
    );
...
test('the actual rendering z-index should satisfy the logic order of their indexes', async () => {
  ...
  blocks
    .filter(block => block.flavour !== 'lovenotes:surface')
    .forEach((block, index) => {
