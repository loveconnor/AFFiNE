 ...
 *
 * To add Turbo Rendering support for a new block type (e.g., 'lovenotes:my-block'):
 *
 * 1.  **In the block's package (e.g., `blocksuite/lovenotes/blocks/my-block`):**
 *   a.  Add `@blocksuite/lovenotes/gfx/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
 *   b.  Implement the Layout Handler (e.g., `MyBlockLayoutHandlerExtension`) and Painter Worker (e.g., `MyBlockLayoutPainterExtension`). Refer to `ParagraphLayoutHandlerExtension` and `ParagraphLayoutPainterExtension` in `blocksuite/lovenotes/blocks/block-paragraph` for implementation examples.
 ...
 *   e.  Add a TypeScript project reference to `blocksuite/lovenotes/gfx/turbo-renderer` in `tsconfig.json`.
 *
 ...
 *
 * 3.  **Run `yarn lovenotes init`** from the workspace root to update generated configuration files (`workspace.gen.ts`) and the lockfile (`yarn.lock`).
 *
 * **Note:** Always ensure the directory structure and export patterns match the `paragraph` block (`blocksuite/lovenotes/blocks/block-paragraph`) for consistency.
 */
export class ViewportTurboRendererExtension extends GfxExtension {
  ...
  override mounted() {
    const mountPoint = document.querySelector('.lovenotes-edgeless-viewport');
    if (mountPoint) {
