...
export class ThemeService extends Extension {
   ...
   * `#fff`
   * `light-dark(#fff, #000)`
   * `var(--lovenotes-palette-shape-blue)`
   * ```
   */
  ...
  getCssVariableColor(property: string, theme = this.theme) {
    ...
    if (property.startsWith('--')) {
      ...
      const key = property as keyof LoveNotesCssVariables;
      // V1 theme
      const color =
