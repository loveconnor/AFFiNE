...
export class DetailSelection {
  ...
  focusFirstCell() {
    const firstId = this.viewEle.querySelector('lovenotes-data-view-record-field')
      ?.column.id;
    if (firstId) {
      this.selection = {
        propertyId: firstId,
  ...
  getFocusCellContainer(selection: DetailViewSelection) {
    return this.viewEle.querySelector(
      `lovenotes-data-view-record-field[data-column-id="${selection.propertyId}"]`
  ...
  getSelectCard(selection: KanbanCardSelection) {
    ...
    return this.viewEle
      .querySelector(`lovenotes-data-view-kanban-group[data-key="${groupKey}"]`)
      ?.querySelector(
        `lovenotes-data-view-kanban-card[data-card-id="${cardId}"]`
