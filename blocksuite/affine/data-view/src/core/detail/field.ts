...
export class RecordField extends SignalWatcher(
  ...
  static override styles = css`
    ...
    lovenotes-data-view-record-field .icon {
      display: flex;
      align-items: center;
      width: 16px;
      height: 16px;
    ...
    lovenotes-data-view-record-field .icon svg {
      width: 16px;
      height: 16px;
    ...
    .field-content .lovenotes-database-number {
      text-align: left;
  ...
  changeEditing = (editing: boolean) => {
    const selection = this.closest('lovenotes-data-view-record-detail')?.selection;
    if (selection) {
      selection.selection = {
...
declare global {
  interface HTMLElementTagNameMap {
    'lovenotes-data-view-record-field': RecordField;
  }
}
