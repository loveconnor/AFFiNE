...
export class KanbanDragController implements ReactiveController {
  dragStart = (ele: KanbanCard, evt: PointerEvent) => {
    ...
    const currentGroup = ele.closest('lovenotes-data-view-kanban-group');
      ...
      this.host.disposables.add(
        this.logic.handleEvent('dragStart', context => {
          ...
          if (target instanceof Element) {
            const cell = target.closest('lovenotes-data-view-kanban-cell');
            if (cell?.isEditing$.value) {
              return;
            }
            cell?.selectCurrentCell(false);
            const card = target.closest('lovenotes-data-view-kanban-card');
            if (card) {
...
const createDragPreview = (card: KanbanCard, x: number, y: number) => {
  ...
  kanbanCard.style.backgroundColor = 'var(--lovenotes-background-primary-color)';
...
const createDropPreview = () => {
  ...
  div.style.backgroundColor = 'var(--lovenotes-primary-color)';
