...
export class TableRowView extends SignalWatcher(
  ...
  static override styles = css`
    .lovenotes-database-block-row:has(.row-select-checkbox.selected) {
      background: var(--lovenotes-primary-color-04);
    }
    .lovenotes-database-block-row:has(.row-select-checkbox.selected)
      .row-selected-bg {
      position: relative;
    ...
    .lovenotes-database-block-row.selected > .database-cell {
    ...
    .lovenotes-database-block-row .show-on-hover-row {
      visibility: hidden;
      opacity: 0;
    }
    .lovenotes-database-block-row:hover .show-on-hover-row {
      visibility: visible;
      opacity: 1;
    }
    .lovenotes-database-block-row:has(.active) .show-on-hover-row {
      visibility: visible;
      opacity: 1;
    }
    .lovenotes-database-block-row:has([data-editing='true']) .show-on-hover-row {
      visibility: hidden;
      opacity: 0;
    }
    ...
    .row-op {
      ...
      padding: 4px;
      border-radius: 4px;
      box-shadow: var(--lovenotes-button-shadow);
  ...
  get groupKey() {
    return this.closest<TableGroup>('lovenotes-data-view-table-group')?.group?.key;
  }
  ...
  override connectedCallback() {
    ...
    this.classList.add('lovenotes-database-block-row', 'database-row');
  }
