...
export const getTableGroupRect = (ele: HTMLElement) => {
  const group = ele.closest('lovenotes-data-view-virtual-table-group');
  if (!group) {
    return;
  }
  ...
  const top =
    group
      .querySelector('.lovenotes-database-column-header')
      ?.getBoundingClientRect().top ?? groupRect.top;
  const bottom =
    group.querySelector('.lovenotes-database-block-rows')?.getBoundingClientRect()
