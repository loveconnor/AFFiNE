...
import { GenerateDocUrlProvider } from '@blocksuite/lovenotes/shared/services';
import { createDefaultDoc } from '@blocksuite/lovenotes/shared/utils';
import { ShadowlessElement } from '@blocksuite/lovenotes/std';
import type { Doc, Workspace } from '@blocksuite/lovenotes/store';
...
export class DocsPanel extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    ...
    .delete-doc-icon svg {
      width: 14px;
      height: 14px;
      color: var(--lovenotes-secondary-color);
      fill: var(--lovenotes-secondary-color);
    }
    .new-doc-button {
      margin-bottom: 16px;
