...
export const synced: InitFn = (collection: Workspace, id: string) => {
  ...
  docSyncedPage.load(() => {
    ...
    const rootId = docSyncedPage.addBlock('lovenotes:page', {
      title: new Text('Synced - Page View'),
    });

    docSyncedPage.addBlock('lovenotes:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedPage.addBlock('lovenotes:note', {}, rootId);
  ...
  docSyncedEdgeless.load(() => {
    // Add root block and surface block at root level
    const rootId = docSyncedEdgeless.addBlock('lovenotes:page', {
      title: new Text('Synced - Edgeless View'),
    });

    docSyncedEdgeless.addBlock('lovenotes:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedEdgeless.addBlock('lovenotes:note', {}, rootId);
  ...
  docMain.load(() => {
    // Add root block and surface block at root level
    const rootId = docMain.addBlock('lovenotes:page', {
      title: new Text('Home doc, having synced blocks'),
    });

    const surfaceId = docMain.addBlock('lovenotes:surface', {}, rootId);
    const noteId = docMain.addBlock('lovenotes:note', {}, rootId);
