...
import { ShapeTool } from '@blocksuite/lovenotes/gfx/shape';
import type { IPoint, IVec } from '@blocksuite/lovenotes/global/gfx';
...
import type { ToolOptions } from '@blocksuite/lovenotes/std/gfx';
...
export async function getNoteRect(page: Page, noteId: string) {
  const xywh: string | null = await page.evaluate(
    ([noteId]) => {
      ...
      if (block?.flavour === 'lovenotes:note') {
...
export async function switchMultipleEditorsMode(page: Page) {
  await page.evaluate(() => {
    const containers = document.querySelectorAll('lovenotes-editor-container');
...
export function locatorEdgelessComponentToolButton(
  ...
  const button = page
    .locator('lovenotes-toolbar-widget editor-toolbar editor-icon-button')
    .filter({
      hasText: text,
    });
...
export async function getEdgelessSelectedRect(page: Page) {
  const selectedBox = await page.evaluate(() => {
    const selected = document
      ...
      ?.shadowRoot?.querySelector('.lovenotes-edgeless-selected-rect');
    if (!selected) {
...
export async function getAllNoteIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll('lovenotes-note')).map(
      note => note.model.id
    );
  });
}
...
export async function getAllEdgelessNoteIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll('lovenotes-edgeless-note')).map(
      note => note.model.id
    );
  });
}
...
export async function getAllEdgelessTextIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll('lovenotes-edgeless-text')).map(
      text => text.model.id
    );
  });
}
...
export function locatorScalePanelButton(page: Page, scale: number) {
  return page.locator('lovenotes-size-dropdown-menu').getByLabel(String(scale));
}
...
export async function multiTouchDown(page: Page, points: Point[]) {
  await page.evaluate(points => {
    const target = document.querySelector('lovenotes-edgeless-root');
    if (!target) {
      throw new Error('Missing edgeless page');
    }
...
export function locatorComponentToolbar(page: Page) {
  return page.locator('lovenotes-toolbar-widget editor-toolbar');
}
...
export async function resizeConnectorByStartCapitalHandler(
  ...
  const handler = page.locator(
    '.lovenotes-edgeless-selected-rect .line-controller.line-start'
  );
...
export function getFrameTitle(page: Page, frame: string) {
  return page.locator(`lovenotes-frame-title[data-id="${frame}"]`);
}
